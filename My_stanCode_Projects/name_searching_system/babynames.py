...
def add_file(name_data, filename):
    ...
    with open(filename, 'r') as f:
        for line in f:
            ...
            # Find year in first line
            if len(name_info) == 1:
                year = name_info[0]
                year = year.strip()  # Remove space
            else:
                rank = name_info[0]
                rank = rank.strip()
                name1 = name_info[1]
    ...
    for name in name_data:
        ...
        if target in name:
            ...
            for i in range(len(name)):
                if i == 0:  # First alphabet
                    first_ab = name[i].upper()
                    name0 += first_ab
                else:
                    name0 += name[i]
            # Add all related names into the names dictionary
            names.append(name0)
    return names
